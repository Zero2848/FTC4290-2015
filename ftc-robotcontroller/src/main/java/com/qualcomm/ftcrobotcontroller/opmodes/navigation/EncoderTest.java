...
public class EncoderTest extends OpMode implements NavXDataReceiver {
    ...
    private static final double NAVX_TARGET_ANGLE_DEGREES = 45.0;    //target angle for PID
    private static final double ROT_PID_P = 0.01; //0.009; //.01
    private static final double ROT_PID_I = 0.0; //0.0002; //.0005
    private static final double ROT_PID_D = 0.005; //0.11; //.05
    ...
    private static final double DISTANCE_FEET = 0;              //distance in feet
    private static final double MIN_POWER = 0;              //distance in feet

    private static final double FORWARD_ROTATION_FEROCITY = 1; //how ferociously should we rotate? - lower if oscillating
    ...
    public void init() {
        ...
        yawPIDController.setPID(ROT_PID_P, ROT_PID_I, ROT_PID_D);
    ...
    public void loop() {
        ...
        if (yawPIDController.isUpdateAvailable(yawPIDState)) {
            telemetry.addData("NavX Debug Coefficients", yawPIDController.getCoefficientDebugString());
            DbgLog.error(yawPIDController.getCoefficientDebugString());
